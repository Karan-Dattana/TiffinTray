...
const PropertyDetail = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const data = await request(`/tiffin/find/${id}`, "GET")
  ...
  const handleContactOwner = async (e) => {
    ...
    emailjs.sendForm("service_i2ivkjz", "template_bdt7car", formRef.current, 'eLEk1oidY0X2UyjA3')
      .then((result) => {
        handleCloseForm()
        setSuccess(true)
        setTimeout(() => {
  ...
  return (
    ...
    <div className={classes.container}>
      ...
      <h3 style={{ textAlign: 'center', marginBottom: '2.5rem', fontSize: '32px', marginTop: '-2.5rem' }}>Tiffin Details</h3>
        ...
        <div className={classes.right}>
          ...
          <div className={classes.details}>
            <div className={classes.typeAndContinent}>
              <div>Type: <span>{`${propertyDetail?.category}`}</span></div>
              <div>Location: <span>{`${propertyDetail?.location}`}</span></div>
            </div>
            <div className={classes.priceAndOwner}>
              <span className={classes.price}><span>Price:  </span>{`${propertyDetail?.price}`} .rs</span>
            ...
            <div className={classes.moreDetails}>
              ...
              <span>Posted At :{propertyDetail?.createdAt}  <FaSquareFull className={classes.icon} /></span>
            </div>
          </div>
      ...
      {success && (
        <div className={classes.success}>
          You've successfully contacted the tiffin provider!
        </div>
      )}
      {shortComment && (
        <div>
